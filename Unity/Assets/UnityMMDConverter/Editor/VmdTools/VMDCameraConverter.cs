...
public class VMDCameraConverter
{
	...
	// 新增路径配置字段
	private string cameraRootPath = "Camera_root";
	private string distancePath = "Camera_root/Camera_root_1";
	...
	void CreateKeysForCamera(VMDFormat format, AnimationClip clip)
	{
		...
		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalPosition.x", posX_curve);
		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalPosition.y", posY_curve);
		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalPosition.z", posZ_curve);

		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalRotation.x", rotX_curve);
		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalRotation.y", rotY_curve);
		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalRotation.z", rotZ_curve);
		clip.SetCurve(cameraRootPath, typeof(Transform), "m_LocalRotation.w", rotW_curve);

		clip.SetCurve(distancePath, typeof(Transform), "m_LocalPosition.z", dis_curve);
		clip.SetCurve(cameraComponentPath, typeof(Camera), "field of view", fov_curve);
	}
